...
<div class="container">
    ...
    <main>
        <table class="table01">
            ...
            while ($row = $result->fetch_assoc()) {
                $sum +=  $row['Ord_qty'] * (float) $row['Product_Price'];
            ?>
                <tr>
    ...
    <div>
        <h3>Total Of the Cart: Rs <b><?php echo number_format($sum, 2); ?></b></h3>
        <button class="success-btn1" onclick="window.location.href='Handlers/carttoorder.php?key=<?php echo $userid; ?>'">Place Orders</button>
        <button class="cancel-btn1" onclick="window.location.href='Handlers/delcart.php?key=<?php echo $userid; ?>'">Empty Cart</button>
    </div>
